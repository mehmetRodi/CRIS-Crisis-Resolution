...
| Team assignment mutation             | `amplify/functions/assign-team` implements the guarded `assignTeam` mutation (COORDINATOR/ADMIN-only): version-checked `Report.assignedTeamId` update, a new `Assignment` record, and an `ASSIGNED` audit event in one transaction. Wired into the incident-detail panel (`lib/assign-team.ts` + `useAssignTeam`, team picker via `useTeams`), independent of the CRIS-18 status controls (CRIS-32, ADR-0045). Merging duplicate report groups remains deferred (no agreed contract, ADR-0038).                                                                                                                                                                                                                                                                                                                                                                                                             |
