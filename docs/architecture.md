...
| Async Bedrock pipeline               | DynamoDB Stream → EventBridge Pipe → SQS (+ DLQ) → Lambda → Bedrock Triage Agent is wired. The worker selects the Amazon Location Places geocoder by default, has the `geo-places:Geocode` grant, and derives geohashes through shared code; the feature flag provides a null-geocoder fallback. Conservative duplicate detection (§5.4.3, ADR-0037) runs after the durable write as a best-effort step: candidates come from the geohash GSI, are scored `Ds = 0.40·L + 0.25·T + 0.20·G + 0.15·E`, and strong matches (≥ 0.80) share a `duplicateGroupId`; the index name is unverified against a deployed environment. Citizen-alert enqueueing is deferred.                                                                                                                             |
