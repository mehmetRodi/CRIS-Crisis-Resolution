...
| React SPA client                     | `apps/web` has landing, sign-in/sign-up/confirmation, `/report`, `/coordinator`, `/volunteer`, and `/map` routes. The coordinator dashboard renders the priority-ordered incident queue with interactive category/status/region filtering applied client-side over the loaded feed (CRIS-22, ADR-0032); selecting a row opens the incident-detail interface and its guarded transition controls (CRIS-23, ADR-0033). The volunteer route joins explicit, PII-free `Report`/`Assignment`/`Team` selections into a refresh-driven five-lane regional task board with region/category/urgency filters (CRIS-33, ADR-0040). It is deliberately read-only until team/region-scoped guarded volunteer mutations exist, and it does not expose report media (ADR-0039). Both operational routes require a session to read data but do not yet enforce a Cognito group at the route boundary.                       |
...
| State transition mutation            | `amplify/functions/transition-report` implements role-checked, version-checked status transitions with audit events. The coordinator dashboard now invokes `updateReportStatus` from the incident-detail panel (client wrapper `lib/transition-report.ts` + `useReportTransition`), offering coordinator-legal moves, passing the read `version` as the optimistic lock, and surfacing `CONFLICT` (CRIS-18, ADR-0028). Role-scoped affordances by caller group are deferred to CRIS-24.                                                                                                                                                                                                                                                                                                                                                                                                                     |
