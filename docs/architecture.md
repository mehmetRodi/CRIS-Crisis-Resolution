...
| Public projection + real-time        | `PublicReport` is the redacted projection. Both direct report writers call `publishReportUpdate` after their durable writes: `classify-report` and `transition-report` hold schema-level IAM `mutate` grants, and publish failures never roll back DynamoDB/audit commits. Amplify also requires a per-operation rule, so `ADMIN` remains the narrow client-facing mutation gate (CRIS-19, ADR-0009/0029/0030/0048). Three authenticated custom subscriptions provide unfiltered, region-filtered, and status-filtered streams. The coordinator uses the unfiltered event as an invalidation signal and staff-reads only the affected report so `version` and internal detail stay authoritative; the volunteer board reconciles the redacted projection locally. Anonymous map streaming is deferred with its missing public baseline query/marker overlay because Identity Pool guest auth is unsupported on the required custom JS subscription handlers.                                                                               |
