...
| Async Bedrock pipeline               | DynamoDB Stream → EventBridge Pipe → SQS (+ DLQ) → Lambda → Bedrock Triage Agent is wired. An Amazon Location Places geocoder and shared geohash encoder exist, but the worker still selects the null geocoder and lacks Places IAM; deduplication and citizen-alert enqueueing are deferred. |
