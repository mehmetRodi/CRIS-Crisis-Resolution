...
| Public projection + real-time        | `PublicReport` is the redacted projection. The `classify-report` worker receives schema-level IAM mutation access and calls `publishReportUpdate` best-effort after its durable write; the operation also retains an `ADMIN` group rule because Amplify requires operation-level auth on Lambda handlers (ADR-0030). Its argument type contains only public fields. The AI `summary` is persisted on `Report` and flows through both paths. Custom subscriptions that consume the mutation are still disabled (CRIS-28), so nothing subscribes yet; dashboard data remains a one-shot authenticated `Report.list` read with local redaction.                                                                                                                                                                                                                                                                |
