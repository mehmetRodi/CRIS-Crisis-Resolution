...
| Async Bedrock pipeline               | DynamoDB Stream → EventBridge Pipe → SQS (+ DLQ) → Lambda → Bedrock Triage Agent is wired. The agent exposes a geocoding tool, but its production dependency is currently a null geocoder; deduplication and citizen-alert enqueueing are deferred.       |
...
| Map                                 | `apps/web/src/surfaces/map` provides a lazy-loaded MapLibre base map using `VITE_MAP_STYLE_URL` or public demo tiles. Amazon Location tiles/geocoding and incident markers/clustering are not wired.                                                      |
