...
| `AlertSubscription` | Owner in `userId`: owner-scoped operations; `COORDINATOR`/`ADMIN`: read | `subscriptionsByRegion`, `subscriptionsByUser`, `subscriptionsByGeohashPrefix`. SMS/email dispatch is active; PUSH is deferred.                                                  |
| `AlertDelivery`     | `COORDINATOR`/`ADMIN`: read                                             | `deliveriesByReport`. One retry-safe delivery record is maintained per report, recipient, and channel.                                                                           |
