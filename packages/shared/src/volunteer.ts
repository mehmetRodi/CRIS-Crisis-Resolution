...
/** Workflow lanes exposed by the read-only volunteer task projection. */
...
/**
 * The complete server-returned volunteer task shape.
 *
 * Deliberately excludes raw report text, reporter identity/contact, media,
 * internal notes, and precise coordinates/geohashes.
 */
...
/** Minimal internal records accepted by the server-side projection builder. */
...
function newestAssignmentsByReport(
  ...
  for (const assignment of assignments) {
    ...
    if (!current || timestamp(assignment.createdAt) >= timestamp(current.createdAt)) {
      newest.set(assignment.reportId, assignment);
    }
  }
  return newest;
}

/** Join strictly projected model records into the redacted volunteer task response. */
